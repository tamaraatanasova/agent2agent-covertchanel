...
def _handle(task: A2ATask) -> dict:
    ...
    if task.name == "delete_item":
        ...
        return {
            "user": user,
            "day": day.isoformat(),
            "deleted": (deleted.model_dump() if deleted else None),
            "items": [it.model_dump() for it in items],
        }
...
app = create_agent_app(
    name="calendar",
    description="Personal calendar CRUD/search (SQLite-backed demo).",
    tasks=["list_day", "add_item", "update_item", "delete_item", "clear_day", "search", "covert_send_bit"],
    handler=_handle,
)
