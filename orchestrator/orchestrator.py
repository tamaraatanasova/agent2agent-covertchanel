...
from orchestrator.remote_a2a import send_envelope
...
from shared.a2a_types import A2AEnvelope, A2ATask, MessageType
...
class Orchestrator:
        ...
        *,
        ...
        try:
            if USE_REMOTE_AGENTS:
                ...
                upstream = send_envelope(url, env)
    ...
    def run_case(self, incident_bundle: dict[str, Any]) -> dict[str, Any]:
        ...
        if "error" in ti_r:
            self._store.set_final_report(case_id, {"error": ti_r["error"]})
            return {"case_id": case_id, "error": ti_r["error"]}
        ti = ti_r["output"]
        ...
        if "error" in anomaly_r:
            self._store.set_final_report(case_id, {"error": anomaly_r["error"]})
            return {"case_id": case_id, "error": anomaly_r["error"]}
        anomaly = anomaly_r["output"]
        ...
        if "error" in plan_r:
            self._store.set_final_report(case_id, {"error": plan_r["error"]})
            return {"case_id": case_id, "error": plan_r["error"]}
        plan = plan_r["output"]
        ...
        if "error" in compliance_r:
            self._store.set_final_report(case_id, {"error": compliance_r["error"]})
            return {"case_id": case_id, "error": compliance_r["error"]}
        compliance = compliance_r["output"]
        ...
        if "error" in report_r:
            self._store.set_final_report(case_id, {"error": report_r["error"]})
            return {"case_id": case_id, "error": report_r["error"]}
        report = report_r["output"]
    ...
    def demo_covert(
        ...
        *,
        ...
        self._store.create_case(case_id, {"demo": "covert", "channel": channel, "bits": bits, "bits_len": len(bits), "bits_hash": bits_hash})
        ...
        def run_once(mode: str, *, task_name: str, size_defense: bool) -> dict[str, Any]:
            ...
            for idx, bit in enumerate(bits):
                ...
                r = self._call_agent(
                    ...
                    from_agent="orchestrator",
                    to_agent="malicious",
                    task=A2ATask(name=task_name, parameters={"i": idx, "bit": bit}),
                    mitigation_mode=mode,
                    rate_limit=False,
                )
                timing = r.get("timing") or {}
                per_bit.append({"i": idx, **timing, "output": r.get("output"), "error": r.get("error")})
        ...
        if channel == "timing":
            ...
            if compare:
                ...
                return {"case_id": case_id, "bits_len": len(bits), "bits_hash": bits_hash, "channel": channel, "baseline": baseline, "defended": defended}
            single = run_once("always", task_name=task_name, size_defense=False)
            return {"case_id": case_id, "bits_len": len(bits), "bits_hash": bits_hash, "channel": channel, **single}
        ...
        if channel == "storage":
            ...
            return {"case_id": case_id, "bits_len": len(bits), "bits_hash": bits_hash, "channel": channel, "blocked": blocked, **single}
